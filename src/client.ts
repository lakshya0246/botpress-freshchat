...
class FreshchatClient {
  ...
  private async _getUserByEmail(
    email?: string
  ): Promise<FreshchatUser | undefined> {
    if (!email) {
      return undefined;
    }
    try {
      const result = await this._client.get("/users?id=" + email);
  ...
  private async _getUserByReferenceId(
    referenceId: string
  ): Promise<FreshchatUser | undefined> {
    try {
      const result = await this._client.get(
        "/users?reference_id=" + referenceId
      );
      return result?.data?.users[0];
    } catch (e: any) {
      this._logger
        .forBot()
        .error(
          "Failed to get user by reference_id: " + referenceId,
          e.message,
          e?.response?.data
        );
      throw new sdk.RuntimeError(
        "Failed to get user by reference_id " + e.message
      );
    }
  }
  ...
  public async getChannels(): Promise<FreshchatChannel[]> {
    try {
      const result = await this._client.get<{ channels: FreshchatChannel[] }>(
        "/channels?items_per_page=9999"
      );
      return result?.data?.channels;
    } catch (e: any) {
      this._logger
        .forBot()
        .error("Failed to get channels : " + e.message, e?.response?.data);
  ...
  public async verifyToken(): Promise<boolean> {
    try {
      const result = await this._client.get<{ organisation_id: number }>(
  ...
  public async getAgentById(id: string): Promise<FreshchatAgent | undefined> {
    try {
      ...
      this._logger
        .forBot()
        .error("Failed to get user by id: " + id, e.message, e?.response?.data);
      throw e;
    }
  }
}
