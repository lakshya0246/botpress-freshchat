...
type CommonChannelProps<
  ...
  TIntegration extends BaseIntegration = IP extends sdk.IntegrationProps<
    infer TI
  >
    ? TI
    : never
...
type ToolFactory<
  ...
  >
    ...
    : never
> = (
  props: CommonChannelProps<IP, TIntegration>
...
type InferToolsFromToolset<
  ...
  >
    ...
    : never
> = {
  [Tool in Extract<keyof Toolset, string>]: Toolset[Tool] extends ToolFactory<
    infer ReturnType,
    IP,
    TIntegration
  >
...
export const createChannelWrapper =
  ...
  >() =>
  <
    TOOLSET extends Record<string, ToolFactory<any, IP, TIntegration>>,
    EXTRAMETA extends Record<string, any> = {}
  >({
    toolFactories,
    extraMetadata: _,
  }: {
  ...
  >(
    ...
    channelImpl: (
      props: CFUNCPROPS & InferToolsFromToolset<TOOLSET, IP, TIntegration>
    ) => Promise<void>
  ): CFUNC =>
